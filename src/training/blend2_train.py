...
class Blend2Trainer:
    ...
    class FTTransformer(nn.Module):
        def __init__(self, n_features, d_token, n_blocks, n_heads, dropout):
            super().__init__()
            self.feature_tokenizer = Blend2Trainer.FeatureTokenizer(n_features, d_token)
            self.transformer_blocks = nn.ModuleList([
                Blend2Trainer.TransformerBlock(d_token, n_heads, dropout) for _ in range(n_blocks)
            ])
    ...
    def train():
        ...
        train_fe_df = Blend2Trainer.create_features_from_script_1(train_df.copy())

        # Define target and features
        target_column = 'BlendProperty2'
        ...
        top_features, feature_importance = Blend2Trainer.select_top_features(X_scaled_df, y, feature_cols, n_features_to_select=50)
        ...
        model_dir = Path(__file__).parent.parent.parent / 'models' / 'Blend-2'
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(X_selected, y)):
            ...
            model = Blend2Trainer.FTTransformer(n_features, d_token, n_blocks, n_heads, dropout).to(device)
            ...
            for epoch in range(n_epochs):
                ...
                val_mape = Blend2Trainer.calculate_mape(np.array(val_true), np.array(val_preds))
        ...
        overall_oof_mape = Blend2Trainer.calculate_mape(y.values, oof_predictions)
