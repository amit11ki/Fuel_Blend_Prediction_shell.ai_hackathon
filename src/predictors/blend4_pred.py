...
class Blend4Predictor:
    ...
    class FTTransformer(nn.Module):
        def __init__(self, n_features, d_token, n_blocks, n_heads, dropout):
            super().__init__()
            self.feature_tokenizer = Blend4Predictor.FeatureTokenizer(n_features, d_token)
            self.transformer_blocks = nn.ModuleList([
                Blend4Predictor.TransformerBlock(d_token, n_heads, dropout) for _ in range(n_blocks)
            ])
    ...
    def predict(test_filepath):
        ...
        try:
            test_fe_df = Blend4Predictor.create_features_from_script_1(test_df.copy())
            if test_fe_df.empty:
        ...
        scaler_path = os.path.join(os.path.dirname(__file__), '..', '..', 'models', 'Blend-4', 'scaler.pkl')
        top_features_path = os.path.join(os.path.dirname(__file__), '..', '..', 'models', 'Blend-4', 'top_features.pkl')
        try:
        ...
        model_base_dir = os.path.join(os.path.dirname(__file__), '..', '..', 'models', 'Blend-4')
        for fold in range(1, n_splits + 1):
            model_path = os.path.join(model_base_dir, f'best_fttransformer_model_fold_{fold}.pth')
            try:
                if not os.path.exists(model_path):
                    raise FileNotFoundError(f"Model file not found: {model_path}")
                model = Blend4Predictor.FTTransformer(X_test_selected.shape[1], d_token, n_blocks, n_heads, dropout).to(device)
        ...
        try:
            result = pd.Series(final_test_predictions, index=test_ids, name='BlendProperty4')
            if result.empty:
...
if __name__ == "__main__":
    ...
    test_filepath = '/content/drive/MyDrive/work/sell/fuel_blend_prediction/data/test.csv'
    predictions = Blend4Predictor.predict(test_filepath)
    if predictions is not None:
        print("\n--- Predictions on Test Data (BlendProperty4) ---")
        print(pd.DataFrame({'ID': predictions.index, 'BlendProperty4': predictions}))