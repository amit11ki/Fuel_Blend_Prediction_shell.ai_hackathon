...
class Blend1Predictor:
    @staticmethod
    def predict(test_filepath):
        print("Starting prediction for BlendProperty1...")
        try:
            # Verify files
            model_filename = os.path.join(os.path.dirname(__file__), '..', '..', 'models', 'Blend-1', 'model_BlendProperty1.joblib')
            ...
            result = pd.Series(predictions, index=df_test['ID'], name='BlendProperty1')
            if result.empty:
                raise ValueError("Prediction Series is empty")
            return result
        except Exception as e:
            print(f"Error in Blend1Predictor: {str(e)}")
            return None