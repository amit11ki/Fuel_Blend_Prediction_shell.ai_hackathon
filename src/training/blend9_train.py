...
class Blend9Trainer:
    ...
    class FTTransformer(nn.Module):
        def __init__(self, n_features, d_token, n_blocks, n_heads, dropout):
            super().__init__()
            self.feature_tokenizer = Blend9Trainer.FeatureTokenizer(n_features, d_token)
            self.transformer_blocks = nn.ModuleList([
                Blend9Trainer.TransformerBlock(d_token, n_heads, dropout) for _ in range(n_blocks)
            ])
    ...
    def train():
        ...
        train_fe_df = Blend9Trainer.create_features_from_script_1(train_df.copy())

        # Define target and features
        target_column = 'BlendProperty9'
        ...
        model_dir = Path(__file__).parent.parent.parent / 'models' / 'Blend-9'
        ...
        print(f"Feature columns saved to {model_dir / 'feature_cols.pkl'}")
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(X_scaled_df, y)):
            ...
            X_train_fold, X_val_fold = X_scaled_df.iloc[train_index], X_scaled_df.iloc[val_index]
            ...
            model = Blend9Trainer.FTTransformer(n_features, d_token, n_blocks, n_heads, dropout).to(device)
            ...
            for epoch in range(n_epochs):
                ...
                val_mape = Blend9Trainer.calculate_mape(np.array(val_true), np.array(val_preds))
        ...
        overall_oof_mape = Blend9Trainer.calculate_mape(y.values, oof_predictions)
        ...
        print("\nTraining complete. Models, scaler, and feature columns saved.")
