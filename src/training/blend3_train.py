...
class Blend3Trainer:
    class CFG:
        ...
        # Updated output directory to models/Blend-3
        OUTPUT_DIR = Path(__file__).parent.parent.parent / 'models' / 'Blend-3'
    ...
    def run_training_workflow(X, y, X_test, scaler, params, n_folds, epochs, patience, run_name=""):
        ...
        kf = KFold(n_splits=n_folds, shuffle=True, random_state=Blend3Trainer.CFG.SEED)
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(X_scaled, y)):
            ...
            train_loader = DataLoader(Blend3Trainer.BlendDataset(X_train, y_train), batch_size=params['batch_size'], shuffle=True)
            val_loader = DataLoader(Blend3Trainer.BlendDataset(X_val, y_val), batch_size=params['batch_size'], shuffle=False)
            
            model = Blend3Trainer.FTTransformerV2(
                len(X.columns), params['embed_dim'], params['num_heads'], params['num_layers'], params['dropout']
            ).to(Blend3Trainer.CFG.DEVICE)
            ...
            checkpoint_path = Blend3Trainer.CFG.OUTPUT_DIR / checkpoint_filename
            early_stopping = Blend3Trainer.EarlyStopping(patience=patience, verbose=False, path=checkpoint_path)
            
            swa_model = AveragedModel(model)
            swa_scheduler = SWALR(optimizer, swa_lr=Blend3Trainer.CFG.SWA_LEARNING_RATE)
            swa_start_epoch = int(epochs * Blend3Trainer.CFG.SWA_START_EPOCH_RATIO)
            
            for epoch in range(epochs):
                model.train()
                for features, labels in train_loader:
                    features, labels = features.to(Blend3Trainer.CFG.DEVICE), labels.to(Blend3Trainer.CFG.DEVICE)
                    optimizer.zero_grad()
                ...
                val_loss = 0
                with torch.no_grad():
                    val_loss = sum(loss_fn(model(features.to(Blend3Trainer.CFG.DEVICE)), labels.to(Blend3Trainer.CFG.DEVICE)).item() for features, labels in val_loader)
            ...
            torch.optim.swa_utils.update_bn(train_loader, final_model_for_prediction, device=Blend3Trainer.CFG.DEVICE)
            final_model_for_prediction.eval()
            
            fold_tta_preds = np.zeros(len(X_test_scaled))
            for _ in range(Blend3Trainer.CFG.TTA_STEPS):
                test_loader = DataLoader(Blend3Trainer.BlendDataset(X_test_scaled + np.random.normal(0, Blend3Trainer.CFG.TTA_NOISE_LEVEL, X_test_scaled.shape)), batch_size=params['batch_size'], shuffle=False)
                with torch.no_grad():
                    step_preds = np.concatenate([final_model_for_prediction(features.to(Blend3Trainer.CFG.DEVICE)).cpu().numpy() for features, _ in test_loader])
                fold_tta_preds += step_preds.flatten()
            cv_tta_preds += fold_tta_preds / Blend3Trainer.CFG.TTA_STEPS / n_folds

            # Save final models to the output directory
            final_model_filename = f'final_model_{run_name}_fold{fold+1}.pth'
            final_model_path = Blend3Trainer.CFG.OUTPUT_DIR / final_model_filename
    ...
    @staticmethod
    def train():
        print(f"🚀 Starting DIRECT training workflow for target '{Blend3Trainer.CFG.TARGET_COL}' on device: {Blend3Trainer.CFG.DEVICE}")
        Blend3Trainer.CFG.OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
        print(f"📂 All output files will be saved to the '{Blend3Trainer.CFG.OUTPUT_DIR}/' folder.")
        ...
        train_features_full = Blend3Trainer.create_advanced_features(train_df)
        test_features_full = Blend3Trainer.create_advanced_features(test_df)
        ...
        y = train_features_full[[Blend3Trainer.CFG.TARGET_COL]].copy()
        X_test = test_features_full[feature_cols].copy()
        print("\n🚀 Starting feature selection with LightGBM...")
        lgbm = lgb.LGBMRegressor(random_state=Blend3Trainer.CFG.SEED, n_estimators=200, learning_rate=0.05)
        lgbm.fit(X, y.values.ravel())
        feature_importances = pd.DataFrame({
            'feature': X.columns,
            'importance': lgbm.feature_importances_
        }).sort_values('importance', ascending=False)
        top_features = feature_importances.head(Blend3Trainer.CFG.NUM_TOP_FEATURES)['feature'].tolist()
        ...
        best_params = {
            ...
            'num_layers': 7, 
            'num_heads': 8, 
            'dropout': 0.1331265134854856, 
            'batch_size': 32, 
            'learning_rate': 0.0003210293750495007, 
            'weight_decay': 2.4770031895123893e-05, 
            'gradient_clip_norm': 0.7943566583022752
        }
        ...
        final_predictions, final_model_paths = Blend3Trainer.run_training_workflow(
            X, y, X_test, scaler, best_params,
            Blend3Trainer.CFG.NFOLDS, Blend3Trainer.CFG.FINAL_MODEL_EPOCHS, Blend3Trainer.CFG.FINAL_MODEL_PATIENCE,
        ...
        joblib.dump(scaler, Blend3Trainer.CFG.OUTPUT_DIR / 'scaler.gz')
        print(f"🔧 Scaler saved to {Blend3Trainer.CFG.OUTPUT_DIR / 'scaler.gz'}")
        joblib.dump(top_features, Blend3Trainer.CFG.OUTPUT_DIR / 'top_features.list')
        print(f"📊 Top features list saved to {Blend3Trainer.CFG.OUTPUT_DIR / 'top_features.list'}")
        with open(Blend3Trainer.CFG.OUTPUT_DIR / 'best_params.json', 'w') as f:
            json.dump(best_params, f)
        print(f"⚙ Best hyperparameters saved to {Blend3Trainer.CFG.OUTPUT_DIR / 'best_params.json'}")

        # Create Final Submission in the output directory
        print(f"\n{'='*20} CREATING FINAL SUBMISSION FOR {Blend3Trainer.CFG.TARGET_COL} {'='*20}")
        submission_df = pd.DataFrame({'ID': test_df['ID'], Blend3Trainer.CFG.TARGET_COL: final_predictions})
        submission_path = Blend3Trainer.CFG.OUTPUT_DIR / f'submission_{Blend3Trainer.CFG.TARGET_COL}.csv'
