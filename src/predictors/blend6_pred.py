...
class Blend6Predictor:
    ...
    def predict(test_filepath):
        print("Starting prediction for BlendProperty6...")
        try:
            # Load the trained model
            model_filename = os.path.join(os.path.dirname(__file__), '..', '..', 'models', 'Blend-6', 'model_BlendProperty6.joblib')
            ...
            df_test_fe = Blend6Predictor.apply_feature_engineering(df_test)
            if df_test_fe.empty:
            ...
            result = pd.Series(predictions, index=df_test['ID'], name='BlendProperty6')
            if result.empty:
                raise ValueError("Prediction Series is empty")
            return result
        except Exception as e:
            print(f"Error in Blend6Predictor: {str(e)}")
            return None